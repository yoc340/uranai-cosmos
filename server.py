...
4. 四柱推命が明かす宿命
5. 数秘術・血液型が語る本質
6. 姓名判断（本名が提供された場合のみ。漢字の画数に基づき天格・人格・地格・外格・総格を分析。本名が未提供の場合はこのセクションを完全に省略すること）
7. 恋愛・対人運
8. 仕事・金運
9. 今後のアドバイス

各セクションは <h3>タイトル</h3> と <p>1段落（2〜3文）</p> で簡潔にまとめること。
冗長にならず核心を突いた鑑定を。
神秘的かつ温かみのある日本語で。
姓名判断以外のセクションは必ずすべて出力すること。"""
...
async def divine(req: DivineRequest):
    ...
    # 四柱推命の時間情報
    hour_info = f"出生時刻：{req.birth_hour}" if req.birth_hour != "不明" else "出生時刻：不明（年柱・月柱・日柱の三柱で鑑定）"

    # 姓名判断
    name_info = f"本名：{req.real_name}（姓名判断に使用）" if req.real_name else "本名：未提供（姓名判断は省略）"
